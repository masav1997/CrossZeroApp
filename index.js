window.addEventListener('load', function () {
    ...
    field.addEventListener('click', function handleCellClick(e) {
        if (!e.target.classList.contains('cell') || (e.target.classList.contains('x'))
        ...
        if (winner) {
            if (winner === 'x') {
                winnerMessage.innerHTML = 'The 1-st player is win!';
                document.querySelector('.winner-message').style.color = "red";
            }
            else {
                winnerMessage.innerHTML = 'The 2-nd player is win!';
                document.querySelector('.winner-message').style.color = "blue";
            }
        }
    })
